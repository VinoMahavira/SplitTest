...
function sendDimension(changeID, variantID) {
    if (readCookie('_ga')) {
        if (changeID != 0) {
            dataLayer.push({ 'event':'abTest'});
			//'eventAction': prefix + '-' + changeID + '-' + variantID, 'eventLabel': prefix + '-' + changeID + '-' + variantID, 'eventNonInteraction': 1 });
        } else {
...
var changes = {
    ...
    1: {
        variants: {
            1: {
                execute: function () {
                    window.location = "https://www.thebalibible.com/ultimate-escape";
                 dataLayer.push({ 'event':'abTest'});
                }
            }
        }
